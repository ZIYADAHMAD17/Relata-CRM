...
import { Plus, Search, Globe, MapPin, MoreHorizontal, ExternalLink } from 'lucide-react';
...
const logoColors: Record<string, string> = {
  TC: 'from-primary to-secondary',
  SI: 'from-secondary to-accent',
  GC: 'from-success to-accent',
};
...
const Organizations = () => {
  ...
  const filteredOrgs = orgs.filter(o => o.name.toLowerCase().includes(search.toLowerCase()) || o.industry?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="flex flex-col gap-8 pb-20 md:pb-0 max-w-6xl mx-auto h-full">
          ...
          <DialogContent>
            ...
            <form onSubmit={handleCreateOrg} className="flex flex-col gap-4 mt-4">
              <div className="flex flex-col gap-2">
                ...
                <Input id="name" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="TechCorp" />
              </div>
              <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input id="website" value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} placeholder="techcorp.com" />
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="location">Location</Label>
                <Input id="location" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} placeholder="San Francisco, CA" />
              </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
        <div className="relative max-w-md">
          ...
          <Input placeholder="Search organizations..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 bg-card/50 backdrop-blur-sm border-border/50 rounded-full" />
        </div>
      </motion.div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredOrgs.map((org, i) => (
            ...
            >
              <div className="glass-panel p-6 flex flex-col gap-4 group hover:border-primary/30 hover:-translate-y-1 transition-all cursor-pointer">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${logoColors[org.logo] || 'from-primary to-secondary'} flex items-center justify-center text-white font-bold text-sm shadow-md`}>
                ...
                <div className="flex flex-col gap-2 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-primary hover:underline cursor-pointer">{org.website || 'No website'}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-4 border-t border-border/50">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <span className="font-bold text-base text-success text-xs">{org.revenue || 'N/A'}</span>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <Link to={`/organizations/${org.id}`} className="flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-primary transition-colors">
